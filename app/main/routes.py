...
def post(id):
    ...
    if form.validate_on_submit():  #adding comments
        ...
        if pagin.total % 4 + 1 == 1:            # code that will redirect user to a last comment's page after posting a comment
            pages_num = pagin.pages + 1
        else:
            pages_num = pagin.pages
        return redirect(url_for('.post', id=post.id, page=pages_num))
    return render_template('post.html', post=post, user=user, form=form, comments=comments, page=page, pagin=pagin)
...
def following(id):
    if not current_user.confirmed:
        ...
        page = request.args.get('page', 1, type=int)
        posts = Post.query.join(Follow, Follow.followed_id == Post.user_id).filter(Follow.follower_id == user.id).paginate(page=page, per_page=4)
