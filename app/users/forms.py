...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField('Enter your password', validators=[DataRequired(), 
                                Length(min=6), EqualTo('confirm_password', message='Passwords need to match')])
    confirm_password = PasswordField('Confirm your password', validators=[DataRequired(), EqualTo('password', message=" ")])
...
class UpdateAccount(FlaskForm):
    ...
    email = EmailField('Change email', validators=[Regexp('[^@]+@[^@]+\.[^@]+', message='email not valid')])
    password = PasswordField('Change password', validators=[Length(min=6), EqualTo('confirm_password', message='Passwords need to match')])
...
class RequestResetForm(FlaskForm):
    email = EmailField('Have you forgot your password?', [validators.DataRequired(), validators.Email()])
    submit = SubmitField('Submit')
